...
function openAcceleration() {
  acceleration.style.right = '0';
}

function closeAcceleration() {
  acceleration.style.right = '-440px';
}
...
function ConvertFromTo(sourceForm, targetForm) {
  let propIndex = document.property_form.the_menu.selectedIndex;
  let sourceIndex = sourceForm.unit_menu.selectedIndex;
  let targetIndex = targetForm.unit_menu.selectedIndex;
  
  let sourceFactor = factor[propIndex][sourceIndex];
  let targetFactor = factor[propIndex][targetIndex];
...
/*-------Reset button---------*/
document.getElementById('resetButton').addEventListener('click',resetCoverter);
function resetCoverter() {
